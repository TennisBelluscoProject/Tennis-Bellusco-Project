...
function GoalCard({ goal, isCoach, isMobile, onEdit, onDelete, onStatusChange, onProgressChange, onDragStart }: GoalCardProps) {
  ...
  // Quick-action buttons for mobile: move to next/prev status
  ...
  const quickActionLabel: Record<GoalStatus, string> = {
    planned: '▶ Inizia',
    in_progress: '✓ Completa',
    completed: '',
  };
  ...
  return (
    <>
      <div
        ...
        className={`bg-white rounded-xl border border-gray-100 p-4 shadow-sm transition-all animate-fade-in ${
          isMobile ? '' : 'hover:shadow-md cursor-grab active:cursor-grabbing'
        }`}
      >
            ...
            <div className="flex items-center gap-2">
              {prevStatus && (
                <button
                  ...
                  className="text-[11px] bg-gray-100 text-gray-600 rounded-lg px-2.5 py-1.5 font-medium active:scale-95 transition-transform"
                >
              ...
              {nextStatus && (
                <button
                  ...
                  className="text-[11px] rounded-lg px-2.5 py-1.5 font-medium text-white active:scale-95 transition-transform"
                  style={{
                    backgroundColor: nextStatus === 'completed' ? 'var(--success)' : 'var(--club-blue)',
                  }}
                >
                  {quickActionLabel[goal.status]}
                </button>
              )}
            </div>
          ) : (
        ...
        {STATUS_COLUMNS.map((status) => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 px-2 rounded-lg text-xs font-medium transition-all ${
                isActive
                  ? 'bg-white shadow-sm'
            ...
            >
              ...
              {count > 0 && (
                <span
                  className="text-[10px] font-bold rounded-full px-1.5 py-0.5 min-w-[18px] text-center leading-none mt-0.5"
                  style={
                    isActive
      ...
      >
        {filteredGoals.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 text-center">
            <span className="text-3xl mb-2">{statusIcons[activeStatus]}</span>
...
export function KanbanBoard({ goals, isCoach, onEdit, onDelete, onStatusChange, onProgressChange }: KanbanBoardProps) {
  ...
  return (
    <div>
      ...
      {/* Mobile: Tab view with swipe | Desktop: Kanban columns */}
      {isMobile ? (
        <MobileTabView
          goals={filtered}
          isCoach={isCoach}
